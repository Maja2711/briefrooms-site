...
- After a successful active-market Continuous Discovery run, the workflow explicitly dispatches `stock-trading-v2-production.yml` on `main`. The production workflow's `workflow_run` trigger and the liveness watchdog remain redundant recovery paths; only the production bridge can mutate the canonical portfolio.
