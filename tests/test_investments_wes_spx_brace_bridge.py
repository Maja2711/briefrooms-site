...
class WesSpxBraceBridgeTests(unittest.TestCase):
    def test_warmup_emits_no_opinion(self):
    ...
    def test_active_long_risk_on_is_strong_agreement(self):
        ...
        self.assertEqual(rel["class"], "STRONG_AGREEMENT")
        self.assertTrue(rel["alpha_eligible"])

    def test_active_long_defensive_is_strong_conflict(self):
        state = bridge.brace_specialist_state(active_shadow(0.1))
        pit = bridge.point_in_time_status("2026-08-19T10:00:00+00:00", state)
        rel = bridge.relationship("long", state, pit)
        self.assertEqual(state["stance"], "defensive")
        self.assertEqual(rel["class"], "STRONG_CONFLICT")

    def test_retrospective_brace_state_is_excluded_from_alpha(self):
        state = bridge.brace_specialist_state(
            active_shadow(0.9, updated_at="2026-08-19T11:00:00+00:00")
        )
        pit = bridge.point_in_time_status("2026-08-19T10:00:00+00:00", state)
        rel = bridge.relationship("long", state, pit)
        self.assertFalse(pit["eligible"])
        self.assertEqual(pit["reason"], "brace_state_created_after_wes_decision")
        self.assertEqual(rel["class"], "UNAVAILABLE")
        self.assertFalse(rel["alpha_eligible"])

    def test_governance_failure_cannot_emit_opinion(self):
    ...
    def test_pre_and_post_freeze_baseline_then_wes_without_duplicate(self):
        ...
        self.assertEqual(len(ledger2["records"]), 1)
        row2 = ledger2["records"][0]
        self.assertEqual(row2["v5_counterfactual"]["risk_plan"]["model_version"], "v5-plan")
        self.assertEqual(row2["wes_actual"]["risk_plan"]["model_version"], "WES-1.0.0")
    ...
    def test_actual_outcome_settles_but_v5_counterfactual_stays_pending(self):
        ...
        row = ledger["records"][0]
        self.assertEqual(row["outcome"]["wes_net_result_percent"], 1.25)
        self.assertIsNone(row["outcome"]["incremental_wes_vs_v5_percent"])
        alpha = bridge.build_alpha_report(ledger)
        self.assertFalse(alpha["active_decision_influence"])
        self.assertFalse(alpha["bounded_influence_enabled"])
        self.assertEqual(alpha["by_relationship"]["STRONG_AGREEMENT"]["settled_wes_outcomes"], 1)
        self.assertEqual(alpha["counterfactual"]["resolved_wes_vs_v5_pairs"], 0)

    def test_monitoring_observation_never_enters_alpha(self):
        w = {
            "week_id": "2026-W34",
            "instruments": [
                {
        ...
        row = ledger["records"][0]
        self.assertEqual(row["decision_type"], "no_trade_monitoring")
        self.assertFalse(row["relationship"]["alpha_eligible"])
