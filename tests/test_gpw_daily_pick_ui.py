...
class GpwDailyPickUiTests(unittest.TestCase):
    ...
    def test_active_position_overlay_keeps_open_positions_visible_and_auditable(self):
        ...
        self.assertIn("current?.position", script)
        ...
        self.assertIn('lastMark: "Ostatni kurs"', script)
        self.assertIn('lastMark: "Last mark"', script)
        self.assertIn("mark_is_current", script)
        self.assertIn('activeFromCurrent("us"', script)
        self.assertIn("do końca tygodnia handlowego", script)
        self.assertIn("end of the trading week", script)
