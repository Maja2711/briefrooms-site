...
class GpwDailyPickUiTests(unittest.TestCase):
    ...
    def test_active_position_overlay_keeps_open_positions_visible_and_auditable(self):
        script = (ROOT / "scripts/stock-trading-public.js").read_text(encoding="utf-8")
        refresh = (ROOT / "scripts/stock-trading-live-refresh.js").read_text(encoding="utf-8")
        polish = (ROOT / "pl/inwestycje/stock-trading.html").read_text(encoding="utf-8")
        english = (ROOT / "en/investing/stock-trading.html").read_text(encoding="utf-8")

        self.assertIn("/data/investments/stock_trading_portfolio.json", script)
        self.assertIn("open_positions", script)
        self.assertIn("closed_positions", script)
        self.assertIn("last_mark", script)
        self.assertIn("stop_loss", script)
        self.assertIn("take_profit", script)
        self.assertIn("return_percent", script)
        self.assertIn("stock-trading-public.js", polish)
        self.assertIn("stock-trading-public.js", english)
        self.assertIn("stock-trading-live-refresh.js", polish)
        self.assertIn("stock-trading-live-refresh.js", english)
        self.assertIn("SESSION_STATE_MAX_AGE_MS", refresh)
        self.assertIn("stateAge > SESSION_STATE_MAX_AGE_MS", refresh)
    ...
    def test_us_runtime_installs_shared_core_and_publishes_history_index(self):
        ...
        integrity = (ROOT / "scripts/us_daily_stock_integrity.py").read_text(encoding="utf-8")
        self.assertIn("_rebuild_dedicated_index", integrity)
        self.assertIn("us-daily-stock-history-index-v3", integrity)
        adapter = (ROOT / "scripts/daily_stock_us_adapter.py").read_text(encoding="utf-8")
