...
class BeliefCoreLiveTest(unittest.TestCase):
    ...
    def test_floor_half_hour(self) -> None:
        dt = datetime(2026, 8, 18, 10, 7, tzinfo=NY)
        self.assertEqual(floor_half_hour(dt).time(), time(10, 0))
        dt = datetime(2026, 8, 18, 10, 37, tzinfo=NY)
        self.assertEqual(floor_half_hour(dt).time(), time(10, 30))
    ...
    def test_next_weekday_close_skips_weekend(self) -> None:
        friday = datetime(2026, 8, 21, 16, 7, tzinfo=NY)
        target = next_weekday_close(friday)
    ...
    def test_financial_conditions_majority(self) -> None:
        spec = {
            "kind": "majority_supportive",
            "reference": {"TLT": 100.0, "HYG": 80.0, "UUP": 25.0},
        }
