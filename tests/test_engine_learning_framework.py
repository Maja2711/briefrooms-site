...
class EngineSpecificLearningFrameworkTests(unittest.TestCase):
    ...
    def test_registry_covers_every_public_experiment_and_isolates_state(self) -> None:
        ...
        self.assertEqual(status["engines"], 7)
        self.assertEqual(status["public_experiments_mapped"], 5)
        self.assertEqual(status["isolated_partitions"], 7)
    ...
    def test_run_all_keeps_loops_separate_and_publishes_lessons_only(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(summary["engines"], 7)
            self.assertEqual(summary["delegated_existing_loops"], 2)
            self.assertEqual(summary["engine_local_loops"], 5)
            self.assertEqual(summary["public_experiments_mapped"], 5)
            ...
            self.assertEqual(observatory["coverage"]["public_experiments_total"], 5)
            self.assertEqual(observatory["coverage"]["public_experiments_mapped"], 5)
