...
class WeeklyBrowserLiveContractTests(unittest.TestCase):
    ...
    def test_all_three_stale_quotes_never_blank_a_known_price(self) -> None:
        ...
        self.assertIn("const cardIsEmpty =", stale_branch)
        self.assertIn("const quoteAdvancesBrowserPrice = currentAt > 0 && quoteAt >= currentAt", stale_branch)
        self.assertIn("if (cardIsEmpty || quoteAdvancesBrowserPrice)", stale_branch)
