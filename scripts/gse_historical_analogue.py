...
The v1 GSE forecast remains immutable and authoritative for the GSE shadow
baseline. This module builds a point-in-time historical analogue library and
freezes a separate v2 candidate probability so that v1 vs v2 calibration can be
measured prospectively.

No trade, position, policy, Belief Core or v1 GSE forecast is modified.
"""
...
class HistoricalMarketClient:
    """Small Yahoo daily-history reader, injectable in tests."""
...
def eligible_analogue_rows(
    ...
    *,
    ...
    # Multiple catalogue entries on the same market date can describe one shock.
    # Cap them to one effective market episode to avoid pseudo-replication.
...
def analogue_stats(rows: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    n = len(rows)
    # Beta(2,2) prior keeps tiny historical samples from creating extreme claims.
    shrunk = (hits + 2.0) / (n + 4.0)
    status = "insufficient_sample" if n < MIN_ANALOGUES_FOR_EXPLORATORY else "exploratory" if n < MIN_ANALOGUES_FOR_MEASURING else "measuring"
    return {
        "effective_n": n,
        "status": status,
...
def candidate_from_baseline(
    ...
    weighted: List[Tuple[float, float, int, Dict[str, Any]]] = []
    ...
    for scenario in scenario_snapshot:
        ...
        diag = {"scenario_type": scenario_type, "transmission_weight": impact, **stats}
        scenario_diagnostics.append(diag)
        pa = stats.get("shrunk_direction_probability")
        n = int(stats.get("effective_n") or 0)
        if pa is None or n <= 0:
            continue
        scenario_weight = abs(impact) * float(scenario.get("probability") or 0.0) * float(scenario.get("confidence") or 0.0)
        if scenario_weight > 0:
            weighted.append((scenario_weight, float(pa), n, diag))
    if not weighted:
        return None
    total_weight = sum(x[0] for x in weighted)
    analogue_probability = sum(w * p for w, p, _, _ in weighted) / total_weight
    # Effective N is capped by unique historical market episodes across scenarios.
    effective_event_ids = set()
    for _, _, _, diag in weighted:
        effective_event_ids.update(diag.get("event_ids") or [])
    effective_n = len(effective_event_ids)
    ...
    payload = {
        ...
        "analogue_status": "insufficient_sample" if effective_n < MIN_ANALOGUES_FOR_EXPLORATORY else "exploratory" if effective_n < MIN_ANALOGUES_FOR_MEASURING else "measuring",
        ...
        "formula": "p_v2=p_v1+min(0.20,0.025*N_effective)*(p_analogue-p_v1); clamp[0.50,0.85]",
    ...
    payload["candidate_sha256"] = canonical_sha256({k: v for k, v in payload.items() if k != "candidate_sha256"})
    return payload


def freeze_candidates(state_dir: Path, library: Mapping[str, Any]) -> int:
    baseline_rows = read_jsonl(state_dir / "gse_forecasts.jsonl")
    existing = {str(x.get("baseline_forecast_id")) for x in read_jsonl(state_dir / "gse_v2_forecasts.jsonl")}
    pending = []
...
def verify_candidates(state_dir: Path, now: datetime) -> int:
    ...
    baseline_verifications = {
        str(x.get("forecast_id")): x for x in read_jsonl(state_dir / "gse_verifications.jsonl") if x.get("forecast_id")
    }
    existing = {str(x.get("candidate_id")) for x in read_jsonl(state_dir / "gse_v2_verifications.jsonl")}
    rows = []
...
def _paired_metrics(rows: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "v2_brier_better_rate": round(sum(float(x["candidate_brier"]) < float(x["baseline_brier"]) for x in rows) / n, 6),
    }


def build_calibration(state_dir: Path) -> Dict[str, Any]:
    rows = [x for x in read_jsonl(state_dir / "gse_v2_verifications.jsonl") if bool(x.get("calibration_eligible", True))]
    by_asset = {}
    by_horizon = {}
    ...
    for horizon in sorted({int(x["horizon_hours"]) for x in rows}):
        by_horizon[str(horizon)] = _paired_metrics([x for x in rows if int(x["horizon_hours"]) == horizon])
    report = {
...
def run(state_dir: Path, catalog_path: Path, now: datetime, market: Optional[HistoricalMarketClient] = None) -> Dict[str, Any]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run GSE historical analogue v2 shadow research layer")
