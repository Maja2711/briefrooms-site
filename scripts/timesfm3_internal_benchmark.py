...
def ensure_activation(state_dir: Path, *, now: datetime, bootstrap: bool = False) -> Dict[str, Any]:
    ...
    if isinstance(existing, dict):
        ...
        if existing.get("model_id") != MODEL_ID or existing.get("symbol") != SYMBOL or existing.get("interval") != INTERVAL:
            raise RuntimeError("TimesFM3 benchmark activation contract changed; create an explicit new experiment version")
...
def _forecast_subjects(ledger: Path) -> set[str]:
    return {
        ...
        if row.get("event_type") == "forecast" and str(row.get("source_ref") or "").startswith("timesfm3-benchmark://")
    }
...
def _outcome_subjects(ledger: Path) -> set[str]:
    return {
        ...
        for row in read_events(ledger)
        if row.get("event_type") == "outcome" and str(row.get("source_ref") or "").startswith("timesfm3-benchmark://")
    }
...
def _skip_subjects(ledger: Path) -> set[str]:
    return {
        str(row.get("subject_id"))
        for row in read_events(ledger)
        if row.get("event_type") == "learning_observation" and str(row.get("source_ref") or "").startswith("timesfm3-benchmark://skip/")
    }
...
def _wes_requests(*, activation_at: datetime, weekly_dir: Path) -> list[BenchmarkRequest]:
    ...
    eurusd = next(
        (
            ...
            and (
                str(row.get("instrument_id") or "").lower() == "eurusd"
...
def _resolve_request_horizon(request: BenchmarkRequest, decision_bars: Sequence[Bar]) -> BenchmarkRequest:
    ...
    target = parse_time(request.target_at)
    seconds = (target - origin_completion).total_seconds()
    if seconds <= 0:
...
def discover_requests(
    ...
    *,
    ...
    pending_payload = _pending_payload(state_dir)
    existing = {str(row.get("request_id") or ""): dict(row) for row in pending_payload["requests"] if isinstance(row, Mapping)}

    candidates = _daily_requests(
        activation_at=activated_at,
        daily_spot=daily_spot,
        daily_history=daily_history,
    ) + _wes_requests(activation_at=activated_at, weekly_dir=weekly_dir)
    ...
    for request in candidates:
        ...
        if resolved.target_mode == "completed_30m_bars":
            ...
            target_at = parse_time(str(resolved.target_at))
            target_already_known = bool(bars) and max(_bar_completion(bar) for bar in bars) >= target_at
        if target_already_known:
            append_event(
                ledger,
                event_type="learning_observation",
                occurred_at=iso_z(datetime.now(timezone.utc)),
                subject_id=_subject_id(resolved),
                source_ref=f"timesfm3-benchmark://skip/{resolved.request_id}",
                payload={
                    "producer_schema_version": SCHEMA_VERSION,
                    "request": resolved.as_dict(),
                    "reason": "target_outcome_already_observable_no_historical_backfill",
                    "authority": safety_controls(),
                },
            )
            known.add(_subject_id(resolved))
            skipped_matured += 1
            continue
        existing[resolved.request_id] = resolved.as_dict()
        added += 1

    _write_pending(state_dir, existing.values())
    return added, skipped_matured
...
def settle_outcomes(state_dir: Path, bars: Sequence[Bar]) -> int:
    ...
    for event in read_events(ledger):
        if event.get("event_type") != "forecast":
            continue
        ...
        engine_correct = None if actual_direction == "FLAT" else engine_direction == actual_direction
        benchmark_correct = None if actual_direction == "FLAT" else benchmark_direction == actual_direction
        ...
        append_event(
            ...
            payload={
                ...
                "engine_direction_correct": engine_correct,
                "timesfm3_direction_correct": benchmark_correct,
...
def run_inference(
    ...
    *,
    ...
    for raw in pending["requests"]:
        ...
        if age_at_decision < -1.0 or age_at_decision > MAX_DECISION_ORIGIN_AGE_MINUTES:
            append_event(
                ...
                payload={
                    ...
                    "reason": "decision_origin_stale_or_future",
        ...
        if target_observable:
            append_event(
                ...
                payload={
                    "producer_schema_version": SCHEMA_VERSION,
                    "request": request.as_dict(),
                    "reason": "target_outcome_already_observable_before_inference",
                    "authority": safety_controls(),
                },
            )
            known.add(subject_id)
        ...
        points = runtime.forecast(context, request.horizon_steps)
        ...
        benchmark_direction = _sign_direction(predicted_return)
        append_event(
            ledger,
            event_type="forecast",
            occurred_at=iso_z(now),
...
def _source_metrics(outcomes: Sequence[Mapping[str, Any]], source_engine: str) -> dict[str, Any]:
    rows = [
        ...
        and str(row["payload"].get("source_engine") or "") == source_engine
    ]
    payloads = [row["payload"] for row in rows]
    paired = [
        payload for payload in payloads
        if isinstance(payload.get("engine_direction_correct"), bool)
        and isinstance(payload.get("timesfm3_direction_correct"), bool)
    ]
    disagree = [payload for payload in paired if payload.get("directions_agreed") is False]
    engine_wins_disagree = sum(
        1 for payload in disagree
        if payload["engine_direction_correct"] is True and payload["timesfm3_direction_correct"] is False
    )
    tf_wins_disagree = sum(
        1 for payload in disagree
        if payload["timesfm3_direction_correct"] is True and payload["engine_direction_correct"] is False
    )
    engine_rate = _mean_bool([payload.get("engine_direction_correct") for payload in paired])
    tf_rate = _mean_bool([payload.get("timesfm3_direction_correct") for payload in paired])
    return {
        "paired_resolved": len(paired),
        "engine_direction_hit_rate": engine_rate,
        "timesfm3_direction_hit_rate": tf_rate,
        "hit_rate_delta_engine_minus_timesfm3": (
            engine_rate - tf_rate if engine_rate is not None and tf_rate is not None else None
        ),
        "direction_agreement_rate": _mean_bool([payload.get("directions_agreed") for payload in paired]),
        "disagreement_count": len(disagree),
        "engine_wins_when_disagree": engine_wins_disagree,
        "timesfm3_wins_when_disagree": tf_wins_disagree,
        "both_correct": sum(
            1 for payload in paired
            if payload["engine_direction_correct"] is True and payload["timesfm3_direction_correct"] is True
        ),
        "both_wrong": sum(
            1 for payload in paired
            if payload["engine_direction_correct"] is False and payload["timesfm3_direction_correct"] is False
        ),
    }
...
def verify_state(state_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if activation.get("model_id") != MODEL_ID:
    ...
    for raw in pending["requests"]:
        ...
        decision_at = parse_time(str(raw.get("decision_at") or ""))
        if decision_at <= parse_time(str(activation.get("activated_at") or "")):
            return {"ok": False, "error": "pending_pre_activation"}
    return {
        "ok": True,
        "ledger_count": chain.get("count"),
        "ledger_head_hash": chain.get("head_hash"),
        "pending": len(pending["requests"]),
    }
...
def infer_cycle(
    ...
    *,
    ...
    rows = list(bars) if bars is not None else fetch_bars()
    model = runtime or TimesFM3Runtime()
    appended, expired = run_inference(state_dir, bars=rows, runtime=model, now=now)
    settled = settle_outcomes(state_dir, rows)
