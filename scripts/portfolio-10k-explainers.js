(() => {
  ...
  function apply(root = document) {
    root.querySelectorAll('.position').forEach(card => {
      ...
      card.querySelectorAll('.signal').forEach(signal => {
        signal.title = copy.signals; signal.setAttribute('aria-label', `${signal.textContent.trim()}. ${copy.signals}`);
      });
  ...
  async function applyControlStatus() {
    try {
      ...
      if (!/PROBATIONARY_CONTROL|ACTIVE_PAPER_CONTROL/.test(String(data.controller_status || ''))) return;
