...
            break
        
        # Używamy ai_summary lub title jako tekstu, który wyświetli hotbar.js
        text = (it.get("ai_summary") or it.get("title") or "").strip()
        if not text:
            continue
...
        key = f"v2|{text.replace('|', ' ')}|{dstr}" # Tekst w kluczu
        
        # ########################################################################
        # POPRAWKA: Wartością słownika musi być link (URL) zamiast 'True'
        # Jeśli w JS oczekiwany jest link, musimy go tutaj wstawić.
        out[key] = it["link"] # ZMIENIONO: z True na link
        # ########################################################################
...
# --- DODATKOWA DUBLOWANA LOGIKA PONIŻEJ (ZATRZYMANA DLA PEŁNEJ DŁUGOŚCI) ---
    """
    out = {}
