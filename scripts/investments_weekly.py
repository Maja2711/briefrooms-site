...
The public page is a dashboard, not a raw model dump: every instrument card shows
current price, source symbol, weekly open, change from open, forecast state and
settlement messages that are useful even while the week is still open.
"""
...
def format_price(value: Optional[float]) -> str:
    if value is None:
        return ""
    return f"{value:,.5f}".replace(",", " ").rstrip("0").rstrip(".")
...
def timestamp_label(value: Any, lang: str, fallback: str) -> str:
    ...
    try:
        ...
        return dt.astimezone(TZ).strftime("%Y-%m-%d %H:%M UTC%z")[:-2] + ":" + dt.strftime("%z")[-2:]
    except Exception:
        return text
...
def yahoo_chart_price(symbol: str) -> PricePoint:
    ...
    try:
        ...
        regular = safe_float(result.get("meta", {}).get("regularMarketPrice"))
        if regular is not None:
            return PricePoint(regular, stamp, f"Yahoo Finance:{symbol}:regularMarketPrice")
...
def week_open_point(item: Dict[str, Any], week: Dict[str, Any], inst_cfg: Dict[str, Any]) -> PricePoint:
    ...
    if entry is not None:
        return PricePoint(entry, str(item.get("entry_captured_at") or ""), "weekly forecast entry")
...
def week_has_finished(week: Dict[str, Any]) -> bool:
    end = str(week.get("forecast_for_week_end") or "")
    if not end:
        return False
    try:
        return now_local() >= datetime.fromisoformat(end).replace(tzinfo=TZ, hour=22, minute=0)
    except Exception:
        return False
...
def score_display(item: Dict[str, Any], lang: str) -> str:
    ...
    return str(int(score)) if score is not None else ("Brak score" if lang == "pl" else "No score")
...
def close_display(item: Dict[str, Any], week: Dict[str, Any], lang: str) -> str:
    ...
    return week_in_progress_label(lang) if not week_has_finished(week) else ("Brak danych zamknięcia — sprawdź źródło ceny" if lang == "pl" else "No close data — check price source")
...
def open_change_display(current: Optional[float], weekly_open: Optional[float], lang: str) -> Tuple[str, str]:
    ...
    value = current - weekly_open
    pct = (value / weekly_open * 100.0) if weekly_open else 0.0
    state = "positive" if value > 0 else "negative" if value < 0 else "neutral"
    return f"{value:+.5f} ({pct:+.2f}%)", state


def strategy_move(entry: float, mark: float, direction: str, inst_cfg: Dict[str, Any]) -> Tuple[float, Optional[float]]:
    unit = safe_float(inst_cfg.get("pip_size")) or safe_float(inst_cfg.get("point_size")) or 1.0
    raw = mark - entry
    strategy_raw = raw if direction == "long" else -raw
    return round(strategy_raw / unit, 2), round(strategy_raw / entry * 100.0, 4) if entry else None


def forecast_status(item: Dict[str, Any], week: Dict[str, Any], open_price: Optional[float], lang: str) -> str:
    direction = str(item.get("direction") or "neutral")
    if direction == "neutral":
        return "Brak transakcji" if lang == "pl" else "No trade"
    if open_price is None:
        return "Czeka na cenę otwarcia" if lang == "pl" else "Waiting for weekly open"
    return "Scenariusz aktywny — tydzień trwa" if not week_has_finished(week) and lang == "pl" else "Active scenario — week in progress" if not week_has_finished(week) else ("Tydzień zakończony" if lang == "pl" else "Week closed")


def forecast_outcome(item: Dict[str, Any], week: Dict[str, Any], open_price: Optional[float], inst_cfg: Dict[str, Any], lang: str) -> str:
    direction = str(item.get("direction") or "neutral")
    if direction == "neutral":
        return "Wynik nie dotyczy — brak transakcji" if lang == "pl" else "Not applicable — no trade"
    close = safe_float(item.get("exit_price"))
    if open_price is None:
        return no_week_open_label(lang)
    if close is None:
        return week_in_progress_label(lang) if not week_has_finished(week) else ("Brak danych zamknięcia — nie można policzyć wyniku" if lang == "pl" else "No close data — result cannot be calculated")
    value, pct = strategy_move(open_price, close, direction, inst_cfg)
    pct_text = "" if pct is None else f" / {pct:+.2f}%"
    if abs(value) < 0.05:
        status = "płasko" if lang == "pl" else "flat"
    else:
        status = "zysk" if value > 0 and lang == "pl" else "loss" if value < 0 and lang == "en" else "profit" if value > 0 else "strata"
    return f"{status}: {value:+.2f}{pct_text}"


def is_legal_rationale(text: str) -> bool:
    lowered = text.lower()
    return any(x in lowered for x in ["rekomendacja inwestycyjna", "porada", "educational scenario", "investment advice"])


def render_metric(label: str, value: str) -> str:
    return f'<div class="forecast-metric"><dt>{html.escape(label)}</dt><dd>{html.escape(value)}</dd></div>'
...
def render_instrument_card(item: Dict[str, Any], week: Dict[str, Any], lang: str, inst_cfg: Dict[str, Any], live_prices: Dict[str, Any]) -> str:
    ...
    updated = timestamp_label(rec.get("timestamp") or rec.get("last_attempt_at"), lang, "Brak aktualizacji ceny" if lang == "pl" else "No price update available")
    ...
    metrics = "".join([
        ...
        render_metric("Status", forecast_status(item, week, weekly_open, lang)),
        render_metric("Zamknięcie tygodnia" if lang == "pl" else "Weekly close", close_display(item, week, lang)),
        render_metric("Wynik prognozy" if lang == "pl" else "Forecast result", forecast_outcome(item, week, weekly_open, inst_cfg, lang)),
    ])
    rationale = item.get("rationale_pl" if lang == "pl" else "rationale_en", [])
    rationale = [str(x) for x in rationale if not is_legal_rationale(str(x))]
    rationale_html = ""
    if rationale:
        heading = "Sygnały modelu" if lang == "pl" else "Model signals"
        rationale_html = f'<div class="forecast-rationale"><h4>{heading}</h4><ul>' + "".join(f"<li>{html.escape(x)}</li>" for x in rationale[:4]) + "</ul></div>"
    return f'''
        <article class="forecast-instrument forecast-instrument-card forecast-instrument-card--{dir_class}">
          ...
          <div class="forecast-price-zone"><div class="forecast-price-primary"><span>{'Bieżąca cena' if lang == 'pl' else 'Current price'}</span><strong class="forecast-price-main">{html.escape(current_display)}</strong><small>{'Ostatnia aktualizacja ceny' if lang == 'pl' else 'Last price update'}: {html.escape(updated)}</small></div><div class="forecast-price-secondary"><span>{'Otwarcie tygodnia' if lang == 'pl' else 'Weekly open'}</span><strong>{html.escape(open_display)}</strong><em class="forecast-price-change forecast-price-change--{change_state}">{html.escape(change_text)}</em></div></div>
          <dl class="forecast-metric-grid">{metrics}</dl>{rationale_html}
        </article>'''
...
def render_live_price_panel(method: Dict[str, Any], live_prices: Dict[str, Any], lang: str) -> str:
    ...
    for inst in method.get("instruments", []):
        ...
        cards.append(f'<div class="price-card"><h3>{html.escape(str(label))}</h3><dl><div><dt>{"Bieżąca cena" if lang == "pl" else "Current price"}</dt><dd>{html.escape(price_label)}</dd></div><div><dt>{"Ostatnia aktualizacja ceny" if lang == "pl" else "Last price update"}</dt><dd>{html.escape(updated)}</dd></div><div><dt>{"Źródło ceny" if lang == "pl" else "Price source"}</dt><dd>{html.escape(str(source))}</dd></div></dl></div>')
    return f'<section class="price-panel"><h2>{"Bieżące ceny do wyceny pozycji" if lang == "pl" else "Current prices for position marks"}</h2><div class="price-grid">{"".join(cards)}</div></section>'
...
def capture_live_prices() -> Dict[str, Any]:
    ...
    out = {"updated_at": now_local().isoformat(timespec="seconds"), "prices": {}}
    for inst in method.get("instruments", []):
        inst_id = inst.get("id")
        ...
        if pp.price is not None:
            out["prices"][inst_id] = {"price": pp.price, "timestamp": pp.timestamp, "source": pp.source, "fresh": True, "note": pp.note}
        elif previous.get("price") is not None:
            kept = dict(previous)
            kept.update({"fresh": False, "last_attempt_at": pp.timestamp, "note": pp.note or "brak świeżej ceny"})
            out["prices"][inst_id] = kept
        else:
            out["prices"][inst_id] = {"price": None, "timestamp": pp.timestamp, "source": pp.source, "fresh": False, "note": pp.note or "brak świeżej ceny"}
...
def calculate_result(item: Dict[str, Any], inst_cfg: Dict[str, Any]) -> None:
    ...
    if direction == "neutral":
        ...
        return
    value, pct = strategy_move(entry, exit_, str(direction), inst_cfg)
...
def forecast_instrument(inst: Dict[str, Any], method: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if len(closes) < 55:
        ...
        score = int(max(-100, min(100, (15 if ema20 > ema50 else -15) + (15 if last > ema20 else -15) + ret5 * 3 + ret20 * .8)))
        bullish = int(method.get("decision_rules", {}).get("bullish_threshold", 20))
        bearish = int(method.get("decision_rules", {}).get("bearish_threshold", -20))
        direction = "long" if score >= bullish else "short" if score <= bearish else "neutral"
        trend = f"EMA20 {'powyżej' if ema20 > ema50 else 'poniżej'} EMA50; ostatnia cena {'powyżej' if last > ema20 else 'poniżej'} EMA20"
    return {"instrument_id": inst["id"], "symbol": inst["symbol"], "label_pl": inst["label_pl"], "label_en": inst["label_en"], "direction": direction, "score": score, "confidence": round(min(abs(score) / 100.0, 1.0), 2), "signals": {}, "rationale_pl": [f"Model v{method.get('method_version')} wskazuje: {direction_label(direction, 'pl').lower()}.", f"Trend: {trend}.", f"Momentum: 5 dni {ret5:+.2f}%, 20 dni {ret20:+.2f}%.", "Zmienność neutralna względem ostatnich tygodni."], "rationale_en": [f"Model v{method.get('method_version')} indicates: {direction_label(direction, 'en').lower()}.", "Trend and momentum are calculated from Yahoo Finance daily closes.", f"Momentum: 5 days {ret5:+.2f}%, 20 days {ret20:+.2f}%.", "Volatility is neutral versus recent weeks."], "entry_price": None, "entry_captured_at": None, "exit_price": None, "exit_captured_at": None, "result": None, "result_value": None, "result_percent": None}
...
def make_forecast() -> Path:
    ...
    if existing.get("forecast_created_at"):
        return path
    data = {"week_id": week_id, "method_version": method.get("method_version", "unknown"), "forecast_created_at": now_local().isoformat(timespec="seconds"), "forecast_for_week_start": monday.date().isoformat(), "forecast_for_week_end": friday.date().isoformat(), "timezone": method.get("timezone", "Europe/Warsaw"), "market_window": {"entry_target_local": monday.isoformat(timespec="seconds"), "exit_target_local": friday.isoformat(timespec="seconds")}, "instruments": [forecast_instrument(inst, method) for inst in method.get("instruments", [])]}
    write_json(path, data)
    return path
...
def main() -> None:
    ...
    if args.mode == "auto":
        ...
        make_forecast(); render_pages()
    elif args.mode == "open":
        capture_prices("open"); render_pages()
    elif args.mode == "close":
        capture_prices("close"); render_pages()
