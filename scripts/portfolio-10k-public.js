(() => {
  ...
  const T = lang === 'pl' ? {
    ...
    plannedTitle: 'Portfel oczekuje na pierwszy przebieg danych rynkowych',
    plannedText: 'Nie pokazujemy zmyślonych cen zakupu. Pierwszy udany przebieg modelu zapisze ceny wejścia, kursy walut i liczbę jednostek.',
    ...
    chartTitle: 'Ścieżka wartości', chartText: 'Portfel modelowy kontra globalny ETF benchmarkowy. Wartości obejmują ceny i waluty w PLN.',
    ...
    planned: 'planowana', active: 'aktywna', stock: 'Akcja', etf: 'ETF', approximate: 'wartość orientacyjna'
  } : {
    ...
    plannedTitle: 'Portfolio is waiting for the first market-data run',
    plannedText: 'We do not display invented purchase prices. The first successful model run will freeze entry prices, FX rates and units.',
    ...
    chartTitle: 'Value path', chartText: 'Model portfolio versus the global ETF benchmark. Values include market prices and FX translated into PLN.',
    ...
    method: [
      ['1', 'Valuation', 'Update all instrument prices and USD/PLN, EUR/PLN and DKK/PLN exchange rates.'],
    ...
    planned: 'planned', active: 'active', stock: 'Stock', etf: 'ETF', approximate: 'approximate value'
  };
  ...
  const money = (value, digits = 2) => {
    ...
    return n.toLocaleString(locale, {minimumFractionDigits:digits, maximumFractionDigits:digits}) + ' PLN';
  };
  ...
  function kpi(label, value, sub, cls='') {
    return `<article class="kpi"><small>${esc(label)}</small><strong class="${cls}">${esc(value)}</strong><span>${esc(sub || '')}</span></article>`;
  }

  function renderKpis(data) {
    const ret = num(data.total_return_percent);
    const bench = num(data.benchmark_return_percent);
    const alpha = ret !== null && bench !== null ? ret - bench : null;
    document.getElementById('kpis').innerHTML = [
      kpi(T.currentValue, money(data.total_value_pln ?? data.starting_capital_pln), T.approximate),
      ...
      kpi(T.cash, money(data.cash_pln), `${T.lastUpdate}: ${dateFmt(data.last_updated_at)}`)
    ].join('');
  }
  ...
  function drawChart(data) {
    ...
    if (!snapshots.length) {
      box.innerHTML = `<div class="loading">${esc(data.status === 'planned' ? T.plannedText : T.noData)}</div>`;
      return;
    }
    const width = 1000, height = 270, pad = 34;
    const portfolio = snapshots.map(s => num(s.total_value_pln));
    const benchmark = snapshots.map(s => num(s.benchmark_value_pln));
    ...
    const grid = [0,1,2,3,4].map(i => {
      ...
      return `<line x1="${pad}" y1="${yy}" x2="${width-pad}" y2="${yy}" stroke="rgba(255,255,255,.08)"/><text x="2" y="${yy+4}" fill="#7f95aa" font-size="11">${Math.round(val)}</text>`;
    }).join('');
    box.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(T.chartTitle)}">${grid}<path d="${path(portfolio)}" fill="none" stroke="#52e38b" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="${path(benchmark)}" fill="none" stroke="#7bb8ff" stroke-width="3" stroke-dasharray="8 7" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  }
  ...
  function signalLabel(code) {
    const pl = {
      price_above_ma200:'cena > MA200',ma50_above_ma200:'MA50 > MA200',positive_six_month_momentum:'momentum 6M +',drawdown_below_twenty_percent:'obsunięcie < 20%',contained_short_term_volatility:'zmienność pod kontrolą',
      price_below_ma200:'cena < MA200',ma50_below_ma200:'MA50 < MA200',negative_six_month_momentum:'momentum 6M −',drawdown_above_twenty_percent:'obsunięcie > 20%',elevated_short_term_volatility:'wysoka zmienność',material_news_headline_requires_review:'istotny nagłówek do sprawdzenia'
    };
    const en = {
      price_above_ma200:'price > MA200',ma50_above_ma200:'MA50 > MA200',positive_six_month_momentum:'positive 6M momentum',drawdown_below_twenty_percent:'drawdown < 20%',contained_short_term_volatility:'contained volatility',
      price_below_ma200:'price < MA200',ma50_below_ma200:'MA50 < MA200',negative_six_month_momentum:'negative 6M momentum',drawdown_above_twenty_percent:'drawdown > 20%',elevated_short_term_volatility:'high volatility',material_news_headline_requires_review:'material headline to review'
    };
    return (lang === 'pl' ? pl : en)[code] || code.replaceAll('_',' ');
  }

  function newsList(items) {
    if (!items?.length) return `<li>${esc(T.noNews)}</li>`;
    return items.map(item => `<li><a href="${esc(item.link)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a><span class="news-meta">${esc(item.source || '')}${item.published ? ' · '+esc(item.published) : ''}</span></li>`).join('');
  }

  function positionCard(p) {
    const flag = p.review_flag || 'HOLD';
    const cardTone = flag === 'THESIS_REVIEW' ? 'alert' : flag !== 'HOLD' ? 'review' : '';
    const pnlPct = num(p.pnl_percent);
    ...
    return `<article class="position ${cardTone}">
      <div class="position-head"><div><div class="symbol">${esc(p.broker_symbol)}</div><h3>${esc(p.label)}</h3><div class="type">${esc(p.asset_type === 'ETF' ? T.etf : T.stock)}</div></div><span class="flag ${esc(flag)}">${esc(T.flags[flag] || flag)}</span></div>
      <div class="position-value"><div><small>${esc(T.value)}</small><strong>${esc(money(p.current_value_pln ?? p.entry_value_pln))}</strong></div><div><small>${esc(T.pnl)}</small><b class="${tone(pnlPct)}">${esc(signedPct(pnlPct))} · ${esc(signedMoney(p.pnl_pln))}</b></div></div>
      <div class="metrics">
        <div class="metric"><small>${esc(T.target)}</small><b>${esc(pct(p.target_weight,1))}</b></div>
        <div class="metric"><small>${esc(T.currentWeight)}</small><b>${esc(pct(p.current_weight,1))}</b></div>
        <div class="metric"><small>${esc(T.score)}</small><b>${num(p.model_score) === null ? T.noData : esc(String(p.model_score)+'/100')}</b></div>
        <div class="metric"><small>${esc(T.entry)}</small><b>${price(p.entry_price,p.currency)}</b></div>
        <div class="metric"><small>${esc(T.current)}</small><b>${price(p.current_price,p.currency)}</b></div>
        <div class="metric"><small>${esc(T.quantity)}</small><b>${num(p.quantity) === null ? T.noData : num(p.quantity).toLocaleString(locale,{maximumFractionDigits:6})}</b></div>
        <div class="metric"><small>${esc(T.entryDate)}</small><b>${dateFmt(p.entry_date)}</b></div>
        <div class="metric"><small>${esc(T.dividends)}</small><b>${money(p.dividends_pln)}</b></div>
        <div class="metric"><small>${esc(T.earnings)}</small><b>${dateFmt(p.next_earnings_date)}</b></div>
      </div>
      <p class="thesis">${esc(text(p,'thesis'))}</p>
      <div class="signals">${positiveSignals}${riskSignals}</div>
      <details><summary>${esc(T.details)}</summary><div class="detail-grid">
        <section class="detail-box"><h4>${esc(T.thesis)}</h4><p>${esc(text(p,'thesis'))}</p><h4>${esc(T.invalidation)}</h4><p>${esc(text(p,'invalidation'))}</p></section>
        <section class="detail-box"><h4>${esc(T.risks)}</h4><ul>${riskList.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section>
        <section class="detail-box" style="grid-column:1/-1"><h4>${esc(T.recentNews)}</h4><ul class="news-list">${newsList(p.recent_news)}</ul></section>
      </div></details>
    </article>`;
  }

  function renderPositions(data) {
    document.getElementById('positions').innerHTML = (data.positions || []).map(positionCard).join('');
  }

  function renderReviews(data) {
    const reviews = [...(data.weekly_reviews || [])].reverse().slice(0,12);
    document.getElementById('reviews').innerHTML = reviews.length ? reviews.map(r => `<article class="review"><time>${esc(r.week_id || '')} · ${dateFmt(r.reviewed_at)}</time><p>${esc(lang === 'pl' ? r.summary_pl : r.summary_en)}</p></article>`).join('') : `<div class="loading">${esc(T.noData)}</div>`;
  }

  function renderMethod() {
    document.getElementById('method').innerHTML = T.method.map(([n,h,p]) => `<article class="method-card"><b>${esc(n)}</b><h3>${esc(h)}</h3><p>${esc(p)}</p></article>`).join('');
  }

  function renderAudit(data) {
    const rows = (data.positions || []).map(p => `<tr><td><b>${esc(p.broker_symbol)}</b><br>${esc(p.label)}</td><td>${esc(pct(p.target_weight,1))}</td><td>${dateFmt(p.entry_date)}</td><td>${price(p.entry_price,p.currency)}</td><td>${num(p.quantity) === null ? T.noData : num(p.quantity).toLocaleString(locale,{maximumFractionDigits:6})}</td><td>${money(p.entry_value_pln)}</td><td>${esc(p.status === 'active' ? T.active : T.planned)}</td></tr>`).join('');
    document.getElementById('audit-body').innerHTML = rows;
  }

  function renderStatus(data) {
    const box = document.getElementById('status-note');
    const active = data.status === 'active';
    box.className = 'status-note' + (active ? ' ok' : '');
    box.innerHTML = `<b>${esc(active ? T.activeTitle : T.plannedTitle)}</b><br>${esc(active ? T.activeText : T.plannedText)}`;
    ...
    document.getElementById('source-meta').textContent = `${T.dataSource}: ${data.data_source || T.noData}`;
    document.getElementById('broker-note').textContent = lang === 'pl' ? (data.broker_note_pl || '') : (data.broker_note_en || '');
  }

  function render(data) {
    renderStatus(data); renderKpis(data); drawChart(data); renderAllocation(data); renderPositions(data); renderReviews(data); renderMethod(); renderAudit(data);
  }

  async function load() {
    document.getElementById('positions').innerHTML = `<div class="loading">${esc(T.loading)}</div>`;
    try {
      const response = await fetch('/data/investments/portfolio_10k.json?v=' + Date.now(), {cache:'no-store'});
      if (!response.ok) throw new Error('HTTP ' + response.status);
      const data = await response.json();
      render(data);
    } catch (error) {
      document.getElementById('positions').innerHTML = `<div class="error">${esc(T.loadError)}</div>`;
      document.getElementById('status-note').innerHTML = `<b>${esc(T.loadError)}</b>`;
    }
  }
