(() => {
  ...
  const materialModuleUrl = '/scripts/portfolio-10k-material-reports-public.js?v=1';
  const ensureMaterialModule = () => {
    if (window.BRMaterialReports) return Promise.resolve();
    return new Promise(resolve => {
      const script = document.createElement('script');
      script.src = materialModuleUrl;
  ...
  const T = lang === 'pl' ? {
    ...
    target: 'Cel', currentWeight: 'Udział teraz', entry: 'Cena wejścia', current: 'Cena teraz', quoteTime: 'Notowanie', quantity: 'Liczba jednostek', entryDate: 'Data wejścia', pnl: 'Wynik', dividends: 'Dywidendy', score: 'Wskaźnik przeglądu', earnings: 'Najbliższe wyniki',
    ...
    target: 'Target', currentWeight: 'Current weight', entry: 'Entry price', current: 'Current price', quoteTime: 'Quote time', quantity: 'Units', entryDate: 'Entry date', pnl: 'P/L', dividends: 'Dividends', score: 'Review indicator', earnings: 'Next earnings',
  ...
  function drawChart(data) {
    ...
    if (!snapshots.length) { box.innerHTML = `<div class="loading">${esc(['active','partially_active'].includes(data.status) ? T.noData : T.plannedText)}</div>`; return; }
  ...
  const newsList=items=>!items?.length?`<li>${esc(T.noNews)}</li>`:items.map(item=>{const url=window.BRMaterialReports?.safeHttpsUrl(item.link);const title=url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a>`:esc(item.title);return `<li>${title}<span class="news-meta">${esc(item.source||'')}${item.published?' · '+esc(item.published):''}</span></li>`;}).join('');
  const materialReports=(p,payload)=>window.BRMaterialReports?.renderForPosition({reports:payload?.reports,position:p,lang})||`<section class="material-reports"><div class="material-reports__header"><h4>${esc(lang==='pl'?'Istotne raporty':'Material reports')}</h4></div><p class="material-reports__empty">${esc(lang==='pl'?'Brak nowych istotnych wydarzeń.':'No new material events.')}</p></section>`;

  function positionCard(p,data,materialData) {
  ...
  function renderStatus(data){const box=document.getElementById('status-note'),active=data.status==='active',partial=data.status==='partially_active';const partialTitle=lang==='pl'?'Portfel częściowo aktywny':'Portfolio partially active';box.className='status-note'+(active||partial?' ok':'');box.innerHTML=`<b>${esc(active?T.activeTitle:partial?partialTitle:T.plannedTitle)}</b><br>${esc(active?T.activeText:(text(data,'pending_entry_rule')||T.plannedText))}`;document.getElementById('updated-meta').textContent=`${T.lastUpdate}: ${dateTimeFmt(data.last_updated_at)} · ${T.marketSession}: ${dateFmt(data.last_market_session)}`;document.getElementById('source-meta').textContent=`${T.dataSource}: ${data.data_source||T.noData}`;const note=lang==='pl'?(data.broker_note_pl||''):(data.broker_note_en||'');document.getElementById('broker-note').textContent=note+(lang==='en'?' All portfolio-level values on this page are displayed in USD.':'');}
  function render(data,materialData){renderStatus(data);renderKpis(data);drawChart(data);renderAllocation(data);renderPositions(data,materialData);renderReviews(data);renderMethod();renderAudit(data);}
  async function load(){document.getElementById('positions').innerHTML=`<div class="loading">${esc(T.loading)}</div>`;try{await ensureMaterialModule();const stamp=Date.now();const controlResponse=await fetch('/data/portfolio10k/public/brace_engine_public.json?v='+stamp,{cache:'no-store'}).catch(()=>null);const control=controlResponse?.ok?await controlResponse.json():null;const paperAllowed=['PROBATIONARY_CONTROL','ACTIVE_PAPER_CONTROL'].includes(control?.controller_status);const controlledPublicPath=lang==='en'?'/data/investments/portfolio_10k_usd.json':'/data/investments/portfolio_10k.json';const requestedPath=paperAllowed&&lang==='pl'?control?.portfolio_data_path:null;const portfolioPath=requestedPath==='/data/portfolio10k/paper_portfolio.json'?requestedPath:controlledPublicPath;const [portfolioResponse,reportsResponse]=await Promise.all([fetch(portfolioPath+'?v='+stamp,{cache:'no-store'}),fetch('/data/investments/portfolio_10k_material_reports.json?v='+stamp,{cache:'no-store'}).catch(()=>null)]);if(!portfolioResponse.ok)throw new Error('HTTP '+portfolioResponse.status);const data=await portfolioResponse.json();const materialData=reportsResponse?.ok?await reportsResponse.json():{reports:[]};render(data,materialData);}catch(error){document.getElementById('positions').innerHTML=`<div class="error">${esc(T.loadError)}</div>`;document.getElementById('status-note').innerHTML=`<b>${esc(T.loadError)}</b>`;}}
