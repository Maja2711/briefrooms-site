#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Price-based macro context for the weekly paper model.

The module is deliberately narrow and auditable. It does not interpret news and
never sends orders. For EUR/USD it combines latest available daily observations
for WTI crude oil and the US 10-year Treasury yield. Positive scores support
EUR/USD (weaker USD / stronger EUR); negative scores support EUR/USD shorts.
"""
...
def _change_bps(values: List[float], sessions: int) -> float:
    if len(values) <= sessions:
        return 0.0
    # Yahoo's ^TNX close is the yield in percentage points; 0.01 equals 1 bp.
    return (values[-1] - values[-sessions - 1]) * 100.0


def score_from_observations(
    oil_1w_percent: float,
    oil_4w_percent: float,
    us10y_1w_bps: float,
    us10y_4w_bps: float,
    ...
    oil_1w = -float(weights.get("oil_1w") or 10.0) * clip(
        oil_1w_percent / float(scales.get("oil_1w_percent") or 8.0), -1.0, 1.0
    )
    oil_4w = -float(weights.get("oil_4w") or 5.0) * clip(
        oil_4w_percent / float(scales.get("oil_4w_percent") or 20.0), -1.0, 1.0
    )
    yield_1w = -float(weights.get("us10y_1w") or 10.0) * clip(
        us10y_1w_bps / float(scales.get("us10y_1w_bps") or 20.0), -1.0, 1.0
    )
    yield_4w = -float(weights.get("us10y_4w") or 5.0) * clip(
        us10y_4w_bps / float(scales.get("us10y_4w_bps") or 50.0), -1.0, 1.0
    )
    ...
    return {
        ...
        "score_cap": cap,
        "components": {
            "oil_1w": round(oil_1w, 4),
            "oil_4w": round(oil_4w, 4),
            "us10y_1w": round(yield_1w, 4),
            "us10y_4w": round(yield_4w, 4),
        },
    }
...
def normalized_score(context: Dict[str, Any]) -> float:
    ...
    if cap <= 0:
        return 0.0
    return clip(float(context.get("score") or 0.0) / cap * 100.0, -100.0, 100.0)
...
def context(instrument_id: str, now: datetime, policy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (require_both and (oil is None or us10y is None)) or (oil is None and us10y is None):
        return {
            ...
            "data_quality": "failed",
            "score": 0.0,
            "reason": "required_macro_source_unavailable",
            "errors": {"oil": oil_error, "us10y": yield_error},
    ...
    observations = {
        "oil_1w_percent": round(_return(oil, 5) * 100.0, 4),
        "oil_4w_percent": round(_return(oil, 20) * 100.0, 4),
        "us10y_1w_bps": round(_change_bps(us10y, 5), 4),
        "us10y_4w_bps": round(_change_bps(us10y, 20), 4),
    }
    ...
    return {
        ...
        **scored,
        ...
        "sources": {
            "oil": {"symbol": oil_symbol, "as_of": oil_as_of, "error": oil_error},
            "us10y": {"symbol": yield_symbol, "as_of": yield_as_of, "error": yield_error},
        },
        "as_of": max(str(oil_as_of or ""), str(yield_as_of or "")),
        "evaluated_at": now.isoformat(timespec="seconds"),
        "interpretation": "positive_supports_eurusd_long_negative_supports_eurusd_short",
        "rule": "Oil and US10Y are context inputs, not standalone trade triggers.",
    }
...
def apply_to_candidates(
    ...
    if instrument_id != "eurusd" or macro_context.get("data_quality") != "passed":
        return rows
    ...
    daily_weight = float(blend.get("daily") or 0.35)
    weekly_weight = float(blend.get("weekly") or 0.40)
    macro_weight = float(blend.get("macro") or 0.25)
    ...
    macro_scaled = normalized_score(macro_context)
    ...
    rows["macro_weekly_blend"] = {
        "direction": direction,
        "raw_score": round(combined, 4),
        "conviction": round(abs(combined) * 0.15, 4),
        "base_conviction": round(abs(combined) * 0.15, 4),
        "macro_alignment_adjustment": 0.0,
        "macro_context_score": round(score, 4),
        "inputs": {
            "daily_score": daily_score,
            "weekly_score": weekly_score,
            "macro_score_normalized": round(macro_scaled, 4),
            "weights": {"daily": daily_weight, "weekly": weekly_weight, "macro": macro_weight},
        },
    }
    return rows
...
def position_review(item: Dict[str, Any], macro_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "data_quality": macro_context.get("data_quality"),
        "macro_score": round(score, 4),
        "macro_direction": macro_context.get("direction"),
        "position_direction": side,
        "aligned": aligned,
        "observations": macro_context.get("observations"),
        "rule": "Context is recorded immediately; an open weekly position is changed only by the governed review rules.",
    }
