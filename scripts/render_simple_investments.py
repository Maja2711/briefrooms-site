...
def direction(item: Dict[str, Any]) -> str:
    ...
    if raw in {"long", "short"}: return raw
    score = sf(item.get("score"))
    return "short" if score is not None and score < 0 else "long"


def unit_label(inst: str, lang: str) -> str:
    return "pipsów" if inst == "eurusd" and lang == "pl" else "pips" if inst == "eurusd" else "pkt" if inst == "sp500_futures" and lang == "pl" else "pts" if inst == "sp500_futures" else "USD"


def calc_units(item: Dict[str, Any], mark: Optional[float]) -> Optional[float]:
    entry = sf(item.get("entry_price"))
    if entry is None or mark is None: return None
    move = (mark - entry) if direction(item) == "long" else (entry - mark)
    return move / 0.0001 if str(item.get("instrument_id")) == "eurusd" else move


def calc_value(item: Dict[str, Any], mark: Optional[float]) -> Optional[float]:
    stored = sf(item.get("result_value")) if mark is not None else None
    if stored is not None: return stored
    entry = sf(item.get("entry_price"))
    if entry is None or mark is None or entry == 0: return None
    move = (mark - entry) if direction(item) == "long" else (entry - mark)
...
def result_text(item: Dict[str, Any], lang: str) -> Tuple[str, str]:
    mark = sf(item.get("exit_price"))
    if mark is None: return ("Tydzień trwa" if lang == "pl" else "Week in progress"), "neutral"
    value, units = calc_value(item, mark), calc_units(item, mark)
    pct = sf(item.get("result_percent"))
    if pct is None and sf(item.get("entry_price")):
        entry = sf(item.get("entry_price")) or 1
        move = (mark - entry) if direction(item) == "long" else (entry - mark)
        pct = move / entry * 100
    tone = "positive" if (value or 0) > 0 else "negative" if (value or 0) < 0 else "neutral"
    parts = []
    if value is not None: parts.append(f"{value:+.2f} USD")
    if units is not None: parts.append(f"{units:+.1f} {unit_label(str(item.get('instrument_id') or ''), lang)}")
    if pct is not None: parts.append(f"{pct:+.2f}%")
    return " · ".join(parts) if parts else "—", tone


def status_text(item: Dict[str, Any], lang: str) -> str:
    reason = str(item.get("exit_reason") or "")
    if reason == "stop_loss": return "Stop loss osiągnięty" if lang == "pl" else "Stop loss reached"
    if reason == "take_profit": return "Take profit osiągnięty" if lang == "pl" else "Take profit reached"
    if sf(item.get("exit_price")) is not None: return "Zamknięta" if lang == "pl" else "Closed"
    if item.get("risk_status") == "pending_entry": return "Czekamy na otwarcie" if lang == "pl" else "Waiting for open"
...
def card(item: Dict[str, Any], lang: str) -> str:
    ...
    dirn = direction(item); dir_class = "positive" if dirn == "long" else "negative"
    close = sf(item.get("exit_price")); plan = item.get("risk_plan") if isinstance(item.get("risk_plan"), dict) else {}
    now_price = sf(item.get("exit_observed_price")) or close or sf(item.get("entry_price"))
    res, tone = result_text(item, lang)
    close_txt = fmt_price(close, inst) if close is not None else ("Tydzień trwa" if lang == "pl" else "Week in progress")
    notional = "10 000 EUR" if inst == "eurusd" and lang == "pl" else "10,000 EUR" if inst == "eurusd" else "10 000 USD" if lang == "pl" else "10,000 USD"
    body = "".join([
        metric("Cena otwarcia" if lang == "pl" else "Open", fmt_price(item.get("entry_price"), inst)),
        metric("Cena zamknięcia" if lang == "pl" else "Close", close_txt),
    ...
    return f"<article class='position {dir_class}'><div class='head'><div><p>{'Pozycja edukacyjna' if lang=='pl' else 'Educational position'}</p><h3>{html.escape(label)}</h3><small>Yahoo Finance · {html.escape(str(item.get('symbol') or ''))}</small></div><span class='tag {'long' if dirn=='long' else 'short'}'>{dirn.upper()}</span></div><div class='price-now'><span class='label'>{'Cena teraz' if lang=='pl' else 'Price now'}</span><strong>{html.escape(fmt_price(now_price, inst))}</strong></div><dl class='grid'>{body}</dl></article>"


def render(lang: str) -> str:
    week = latest_week(); items = week.get("instruments", []) if isinstance(week, dict) else []
    closed = [x for x in items if sf(x.get("exit_price")) is not None]
    values = [calc_value(x, sf(x.get("exit_price"))) for x in closed]
    values = [v for v in values if v is not None]
    total = sum(values); wins = sum(1 for v in values if v > 0); losses = sum(1 for v in values if v < 0); win_rate = wins / len(values) * 100 if values else 0
    total_tone = "positive" if total > 0 else "negative" if total < 0 else "neutral"
    week_id = html.escape(str(week.get("week_id") or "")); start = html.escape(str(week.get("forecast_for_week_start") or "")); end = html.escape(str(week.get("forecast_for_week_end") or ""))
    if lang == "pl":
        title = "Otwarte pozycje tygodniowe"; desc = "Prosty widok pozycji tygodniowych. Analizy, re-entry i oceny jakości działają wewnętrznie."
        nav = '<a href="/pl/inwestycje.html">Inwestycje</a><a class="active" href="/pl/inwestycje/prognozy-tygodniowe.html">Pozycje tygodniowe</a><a href="/pl/inwestycje/spx-scenariusze-2026.html">Scenariusze S&P 500</a>'
        home="/pl/"; switch="/en/investing/weekly-forecasts.html"; switch_label="EN"; summary="Podsumowanie bieżącego tygodnia"; total_label="Łączny wynik zamkniętych pozycji w tym tygodniu"; wl="Zyskowne / stratne pozycje"; current="Pozycje tygodnia"; history="Historia od bieżącego tygodnia"; back='<a class="back" href="/pl/inwestycje.html">← Wróć do pokoju Inwestycje</a>'; legal="Treści mają charakter edukacyjny i analityczny. To nie jest rekomendacja inwestycyjna ani porada finansowa."
        table_head="<tr><th>Tydzień</th><th>Instrument</th><th>Pozycja</th><th>Otwarcie</th><th>Zamknięcie</th><th>Powód</th><th>Wynik</th></tr>"
    else:
        title = "Open weekly positions"; desc = "Simple weekly-position view. Internal analysis, re-entry after SL and quality grades stay inside the Investing room."
        nav = '<a href="/en/investing.html">Investing</a><a class="active" href="/en/investing/weekly-forecasts.html">Weekly positions</a><a href="/en/investing/spx-scenarios-2026.html">S&P 500 scenarios</a>'
        home="/en/"; switch="/pl/inwestycje/prognozy-tygodniowe.html"; switch_label="PL"; summary="Current week summary"; total_label="Total closed-position result this week"; wl="Profitable / losing positions"; current="Weekly positions"; history="History from current week"; back='<a class="back" href="/en/investing.html">← Back to Investing room</a>'; legal="Content is educational and analytical. It is not investment advice or financial advice."
