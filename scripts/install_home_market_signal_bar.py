...
GUARD_PATH = "/scripts/home-market-signal-guard-v1.js"
...
GUARD_TAG = f'<script src="{GUARD_PATH}" defer></script>'
...
def patch(source: str) -> str:
    ...
    source = LEGACY_LOCALE_LINK_RE.sub("", source)

    if GUARD_TAG not in source:
        source = source.replace(SCRIPT_TAG, SCRIPT_TAG + "\n" + GUARD_TAG)
...
def validate(source: str) -> None:
    ...
    if source.count("home-market-signal-guard-v1.js") != 1:
        raise RuntimeError("Homepage must load the market signal label guard exactly once")
