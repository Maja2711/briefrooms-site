(() => {
  ...
  function currentArm(arm) {
    return `<article class="abc-arm">
      <div class="abc-arm-head"><span>${esc(arm.arm_id)} · ${esc(arm.label_pl)}</span><b class="${directionClass(arm.direction)}">${esc(arm.direction)}</b></div>
  ...
  function render(payload) {
    ...
    root.innerHTML = `<article class="abc-lab">
      <div class="abc-title-row"><div><span class="abc-eyebrow">PR21 · EUR/USD</span><h3>A/B/C Research Lab</h3><p>Równoległe, prospektywne porównanie trzech silników na tym samym frozen market reference.</p></div><span class="abc-chip">LIVE SHADOW</span></div>
