...
FEEDS = {
    "Polityka / kraj": ["https://tvn24.pl/najnowsze.xml", "https://tvn24.pl/polska.xml", "https://www.polsatnews.pl/rss/wszystkie.xml", "https://www.pap.pl/rss.xml"],
    "Ekonomia": ["https://www.bankier.pl/rss/wiadomosci.xml", "https://www.bankier.pl/rss/gospodarka.xml", "https://www.pap.pl/rss.xml"],
    "Sport": ["https://www.polsatsport.pl/rss/wszystkie.xml", "https://sport.tvp.pl/rss", "https://przegladsportowy.onet.pl/.feed", "https://sportowefakty.wp.pl/rss.xml"],
    "Nauka": ["https://www.esa.int/rssfeed/Our_Activities/Space_Science", "https://www.nasa.gov/news-release/feed/", "https://www.sciencedaily.com/rss/top/science.xml"],
    "Zdrowie": ["https://www.who.int/feeds/entity/mediacentre/news/en/rss.xml", "https://www.cdc.gov/media/rss.htm", "https://www.nhs.uk/news/feed/"],
}

SOURCE_NAMES = [
    (r"tvn24\.pl", "TVN24"), (r"polsatnews\.pl", "Polsat News"), (r"pap\.pl", "PAP"), (r"bankier\.pl", "Bankier.pl"),
    (r"polsatsport\.pl", "Polsat Sport"), (r"sport\.tvp\.pl", "TVP Sport"), (r"przegladsportowy\.onet\.pl|sport\.onet\.pl", "Przegląd Sportowy"),
    (r"sportowefakty\.wp\.pl", "SportoweFakty WP"), (r"nasa\.gov", "NASA"), (r"esa\.int", "ESA"), (r"sciencedaily\.com", "ScienceDaily"),
    (r"who\.int", "WHO"), (r"cdc\.gov", "CDC"), (r"nhs\.uk", "NHS"),
]

BAN = re.compile(r"horoskop|quiz|galeria|plotk|sponsorowany|lotto|eurojackpot|na żywo|relacja live", re.I)
IMG_META_RE = re.compile(r'<meta[^>]+(?:property|name)=["\'](?:og:image|twitter:image)["\'][^>]+content=["\']([^"\']+)["\']', re.I)
IMG_META_RE_ALT = re.compile(r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+(?:property|name)=["\'](?:og:image|twitter:image)["\']', re.I)
EN_HOST_RE = re.compile(r"(nasa\.gov|esa\.int|sciencedaily\.com|who\.int|cdc\.gov|nhs\.uk|reuters\.com|bbc\.|apnews\.com|theguardian\.com)", re.I)
PL_CHARS_RE = re.compile(r"[ąćęłńóśźżĄĆĘŁŃÓŚŹŻ]")
COMMON_EN_RE = re.compile(r"\b(the|and|with|after|before|over|under|from|for|to|of|in|on|as|by|will|is|are|was|were|has|have|says|said|new|world|study|research|science|space|health|risk)\b", re.I)
COMMON_PL_RE = re.compile(r"\b(i|oraz|że|się|jest|są|był|była|będzie|dla|przez|polska|polski|badanie|zdrowie|ryzyko|pokazuje)\b", re.I)
NOISE_RE = re.compile(r"cookie|cookies|reklama|advertisement|subskryb|newsletter|zaloguj|privacy|rodo|wyrażam zgodę|czytaj także|zobacz także", re.I)
...
def sentences(text: str) -> list[str]:
    text = clean_text(text, 2000).replace("…", ".")
    parts = re.findall(r"[^.!?]+[.!?]+|[^.!?]+$", text)
    out = []
    for p in parts:
        p = ensure_period(p)
        if len(p) >= 35 and not NOISE_RE.search(p):
            out.append(p)
    return out


def summary_3_4(text: str, max_len: int = 620) -> str:
    chosen = sentences(text)[:4]
    if not chosen:
        return ensure_period(clean_text(text, max_len))
    result = " ".join(chosen)
    return clean_text(result, max_len)
...
def html_to_article_text(raw_html: str) -> str:
    ...
    for p in paras[:24]:
        t = clean_text(p, 900)
        if len(t) >= 45 and not NOISE_RE.search(t):
            cleaned.append(t)
        if len(cleaned) >= 7:
            break
    return " ".join(cleaned)
...
def translation_cache_key(title: str, link: str) -> str:
    return hashlib.sha256(f"home-pl-v2|{title}|{link}".encode("utf-8")).hexdigest()[:40]
...
def likely_english_item(item: dict) -> bool:
    ...
    if PL_CHARS_RE.search(text) or COMMON_PL_RE.search(text):
        return False
    return len(COMMON_EN_RE.findall(text)) >= 2 and len(COMMON_PL_RE.findall(text)) == 0
...
def apply_translation(item: dict, title_pl: str, summary_pl: str) -> dict | None:
    ...
    if not title_pl or not summary_pl or still_looks_english(title_pl) or still_looks_english(summary_pl):
        return None
    out = dict(item)
    out["title"] = title_pl
    out["summary"] = summary_pl
    out["details"] = summary_pl
...
def translate_english_item_to_polish(item: dict) -> dict | None:
    title, summary, link, source = item.get("title", ""), item.get("summary", ""), item.get("link", ""), item.get("source", "Źródło")
    ...
    if not api_key:
        ...
        return None
    prompt = f'''Przetłumacz i zredaguj anglojęzyczny wpis RSS do polskiej strony BriefRooms. Zwróć wyłącznie JSON:
{{"title_pl":"krótki tytuł po polsku, maks. 92 znaki","summary_pl":"jedno krótkie zdanie po polsku, maks. 150 znaków"}}
...
'''
    try:
        resp = requests.post(
            ...
            json={"model": AI_MODEL, "messages": [{"role": "system", "content": "Jesteś polskim redaktorem BriefRooms. Zwracasz wyłącznie poprawny JSON."}, {"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 260},
            timeout=30,
        )
        resp.raise_for_status()
        raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", resp.json()["choices"][0]["message"]["content"].strip(), flags=re.I | re.S).strip()
        data = json.loads(raw)
        translated = apply_translation(item, str(data.get("title_pl", "")).strip(), str(data.get("summary_pl", "")).strip())
        if translated:
...
def make_item(entry, category: str) -> dict | None:
    ...
    details = summary_3_4(article_text or summary_raw, 620)
    item = {"category": category, "title": title, "summary": summary or "Krótki kontekst i link do źródła.", "details": details or summary, "source": source_name(link), "link": link, "image": entry_image(entry, link), "time": "dzisiaj", "ts": entry_timestamp(entry)}
...
def build_payload(items: list[dict]) -> dict:
    ...
    return {"updated_at": datetime.now().astimezone().isoformat(timespec="minutes"), "count": min(len(items), 24), "latest": strip_ts(latest), "radar": strip_ts(radar)}
