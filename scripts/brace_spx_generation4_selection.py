#!/usr/bin/env python3
"""Diversity-aware, hypothesis-agnostic selection for BRACE-SPX generation 4."""
...
def evidence(row: Mapping[str, Any], mean_correlation: float) -> dict[str, Any]:
    ...
    is_ensemble = archetype in {"consensus", "majority", "trend_breadth", "macro_defense", "veto", "recovery_barbell"}
    ...
    return {
        ...
        "archetype": archetype,
        "variant": str(params.get("variant", "unknown")),
        "is_predeclared_ensemble": is_ensemble,
        ...
        "mean_absolute_correlation_to_pool": round(mean_correlation, 6),
        "diversity_score": round(1.0 - mean_correlation, 6),
        "stable": stable,
    }


def select(experiments: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in experiments:
        ...
        rows.append(evidence(row, mean_corr))
    ...
    selected = min(
        equivalent,
        key=lambda row: (
            row["mean_absolute_correlation_to_pool"],
            row["fold_sharpe_std"],
    ...
    return {
        ...
        "selection_rule": {
            ...
            "tie_break": "lowest mean absolute return correlation, then fold stability, turnover and Sharpe",
...
def run(ledger_path: Path, report_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(experiments, list):
        raise ValueError("Generation 4 ledger must contain an experiments list")
    result = select(experiments)
    audit = {
        ...
        "promotion_reason": "strict gate and explicit human approval are required before any single holdout use",
    }
...
def main() -> None:
    ...
    print(f"BRACE-SPX v4 selection: {audit['selected_candidate_id']}")
